...
def interpret_tree(model, train, test, df_cols):
    ...
    f = shap.force_plot(explainer.expected_value[0], shap_values[0], test, feature_names=df_cols, show=False)
    shap.save_html('../results/force_plot_0.htm', f)
    plt.close()
    
    f = shap.force_plot(explainer.expected_value[1], shap_values[1], test, feature_names=df_cols, show=False)
    shap.save_html('../results/force_plot_1.htm', f)
    plt.close()

    f = shap.force_plot(explainer.expected_value[2], shap_values[2], test, feature_names=df_cols, show=False)
    shap.save_html('../results/force_plot_2.htm', f)
    plt.close()

    f = shap.force_plot(explainer.expected_value[3], shap_values[3], test, feature_names=df_cols, show=False)
